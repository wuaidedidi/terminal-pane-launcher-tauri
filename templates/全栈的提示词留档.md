...
涉及密码哈希时，不要使用未锁版本的 passlib[bcrypt]；优先使用 pbkdf2_sha256 或 pwdlib/argon2。若必须使用 bcrypt，必须显式锁定兼容版本。
...
\- 删除操作触发数据库外键约束错误时，系统返回"系统内部错误"，用户无法了解具体原因，这种情况参考下列方式解决
  ...
  \- 在删除级联数据前检查是否有关联数据，如有则返回友好错误信息（如"该客户有关联数据，无法删除"），提升用户体验，一定要精准而友好，不要类似：操作失败，该数据存在关联记录，无法执行此操作之类太泛化的提示
...
后台页面优先采用浏览器原生文档滚动（body/document scroll），通过 sticky/fixed 实现顶部栏和侧栏；只有在明确需要固定应用壳、让内容区独立滚动时，才采用全高布局（优先 100dvh）+ 单一内部主滚动容器。若采用内部滚动，必须保证完整高度链路、所有中间 flex/grid 父容器设置 min-height: 0 / min-width: 0、主内容区 overflow: auto，并避免多层纵向滚动。不要将 html/body overflow: hidden 或 scrollbar-gutter: stable 设为无条件规则，它们应仅在对应布局和抖动场景下使用。
...
\- docker中前端3000，后端8000，数据库mysql的话3306，其他的按照最通常的端口来，当然prompt中的最优先，如果3000,8000,3306端口被占用,根据项目文件夹的名称进行设置比如207文件夹，前端端口设置为3207，后端端口设置为8207,数据库为2076,其他依次类推，如果数字超过了65535的限制那么可以随机在60000数字向上使用一个随机端口
...
\- 以后在为我生成 docker-compose.yml 时，请务必在顶层显式指定 name 属性，以文件夹名为标准以免重复
...
必须确保中文内容在数据库、接口、页面展示、初始化数据中均不乱码。

实现时需保证：
- 数据库字符集使用 `utf8mb4`
- 服务端和接口统一使用 `UTF-8`
- 初始化脚本使用 `UTF-8` 编码
- 数据库配置在 Docker、应用配置、初始化脚本中保持一致

技术提醒（适用于 MySQL + Java）：
- MySQL 服务端建议显式配置 `utf8mb4`
- JDBC 中 `characterEncoding` 请使用 `UTF-8`
...
为保证 QA 验收稳定性，允许在应用启动时执行初始化逻辑：

- 若 `admin` 不存在，则自动创建
- 若 `admin` 已存在但密码不匹配，则使用当前系统密码加密方式重新修正为 `123456`
- 若 `admin` 存在但角色或状态不正确，则自动修正为启用的管理员
